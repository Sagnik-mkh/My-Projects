...
allBtn.addEventListener(`click`, (event) => {
	...
	const isReqtBtn =
		btn.hasAttribute("data-num-btn") || btn.hasAttribute("data-op-btn");
...
allBtn.addEventListener(`mouseover`, function mouseOverHandler(event) {
	...
	if (tgt.tagName === "BUTTON") {
		...
		if (tgt.getAttribute("data-click") === "inactive") {
			tgt.addEventListener(`mousedown`, function changeStyle() {
				onClick(btnType, tgt);
			});
			tgt.addEventListener(`mouseup`, function reverseStyle() {
				offClick(tgt);
			});
		}
	}
...
allBtn.addEventListener(`mouseout`, function mouseOutHandler(event) {
	...
	if (tgt.tagName === "BUTTON") {
		...
		if (tgt.getAttribute("data-click") === "active") {
			tgt.removeEventListener(`mousedown`, function changeStyle() {
				onClick(btnType, tgt);
			});
			btnOffClickStyle(
				tgt,
...
allBtn.addEventListener("pointerover", function (event) {
	...
	if (tgt.tagName === "BUTTON") {
		const btnType = tgt.getAttribute("data-btn-type");
		if (tgt.getAttribute("data-click") === "inactive") {
			tgt.addEventListener(`pointerdown`, function changeStyle() {
				onClick(btnType, tgt);
			});
			tgt.addEventListener(`pointerup`, function reverseStyle() {
				offClick(tgt);
			});
		}
	}
...
allBtn.addEventListener("pointerout", function (event) {
	...
	if (tgt.tagName === "BUTTON") {
		const btnType = tgt.getAttribute("data-btn-type");
		if (tgt.getAttribute("data-click") === "active") {
			tgt.removeEventListener(`pointerdown`, function changeStyle() {
				onClick(btnType, tgt);
			});
			btnOffClickStyle(
				tgt,
...
clearBtn.addEventListener(`click`, function clearBtn() {
	resetOperation();
});
